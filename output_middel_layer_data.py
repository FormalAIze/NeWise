...
def find_output_bounds(weights, biases, shapes, pads, strides, sizes, types, x0, eps, p_n, activation, method='NeWise', log_name=timestr):
    LBs = [x0-eps]
    UBs = [x0+eps]
    for i in range(1,len(weights)+1):
        ...
        printlog('--------------------' + str(i) + ' layer -------------------', log_name)
        for one in range(LB.shape[0]):
            for two in range(LB.shape[1]):
                for three in range(LB.shape[2]):
                    printlog("{:.5f}, {:.5f}".format(LB[one,two,three], UB[one,two,three]), log_name)
        
    return LBs[-1], UBs[-1], A_u, A_l, B_u, B_l, pad, stride

#Warms up numba functions
def warmup(model, x, eps_0, p_n, activation, func, log_name=timestr):
    print('Warming up...')
    ...
    shapes.append((1,1,1))
    func(weights, biases, shapes, model.pads, model.strides, model.sizes, model.types, x, eps_0, p_n, activation, log_name=log_name)
...
def run_certified_bounds(file_name, n_samples, p_n, q_n, data_from_local=True, method='NeWise', 
    ...
    # warmup(model, inputs[0].astype(np.float32), eps_0, p_n, activation, find_output_bounds, log_name)
    ...
    for i in range(len(inputs)):
        ...
        for j in range(steps):
            ...
            printlog('====================== Overapproximation ======================', log_name)
            LB_total, UB_total, _, _, _, _, _, _ = find_output_bounds(model.weights, model.biases, model.shapes, model.pads, model.strides, model.sizes, model.types, inputs[i].astype(np.float32), np.exp(log_eps), p_n, activation, method, log_name=log_name)
    ...
    printlog("[L0] {:20}-{}, \t total_images={}, \t avg={:.5f}, \t std={:.5f}, \t avg_runtime={:.2f}".format(method, activation, len(inputs), eps_avg, np.std(eps_total), aver_time), log_name)
